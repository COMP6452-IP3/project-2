...
contract retrieveArtworkTestSuite is Licensing {
    ...
    function addArtworksFailTest() public {
        (bool success, bytes memory result) = address(this).delegatecall(abi.encodeWithSignature("addArtwork(uint256,string,uint256)", 3, "Artwork 3", 2003));
        Assert.ok(!success, "WRONG");
        if (success == false) {
    ...
    function addPermissionsFailTest() public {
        (bool success, bytes memory result) = address(this).delegatecall(abi.encodeWithSignature("grantPermission(uint256,address,uint256)", 2, acc1, 10));
        if (success == false) {
            string memory reason = abi.decode(result.slice(4, result.length - 4), (string));
            Assert.equal(reason, "Can only be executed by the artist.", "Failed with unexpected reason");
        } else {
    ...
    function retrieveArtworksTest() payable public {
        (bool success, bytes memory result)  = address(this).delegatecall(abi.encodeWithSignature("getArtwork(uint256)", 1));
    ...
    function retrieveArtworksNotPermittedTest() public {
        (bool success, bytes memory result) = address(this).delegatecall(abi.encodeWithSignature("getArtwork(uint256)", 2));
        Assert.ok(success, "Should not throw any errors.");
        bool successGotArtwork = abi.decode(result, (bool));
